...
def convertTime(time):
    ...
    temp_str = temp_str.replace("Mon", "Mon ")
    temp_str = temp_str.replace("Tue", "Tue ")
    temp_str = temp_str.replace("Wed", "Wed ")
    temp_str = temp_str.replace("Thu", "Thu ")
    temp_str = temp_str.replace("Fri", "Fri ")
    temp_str = temp_str.replace("Sat", "Sat ")
    temp_str = temp_str.replace("Sun", "Sun ")
...
def init():
    ...
    for i in range(len(file_list)):
        ...
        with open('../json/english/' + file_list[i][:-4] + '.json', 'w', encoding='UTF-8-sig') as file:
            file.write(j)
        file.close()
